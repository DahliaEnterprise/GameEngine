...
void tennis_script::start(){objectsInitialized = false;scriptKeepalive = new QTimer();QObject::connect(scriptKeepalive, SIGNAL(timeout()), this, SLOT(wakeupScript()));scriptKeepalive->start(100);playerOneEmblem=new ogleEmblem();playerOnePaddle=new paddle();playerOnePaddle->start(1280,720);playerAiPaddle=new aiPaddle();playerAiPaddle->start(1280,720);ball=new token();ball->start(1280,720);}
